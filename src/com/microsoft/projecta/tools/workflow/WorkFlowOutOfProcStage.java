...
public abstract class WorkFlowOutOfProcStage extends WorkFlowStage {
		...
		public void run() {
			if (mWorkerProc != null) {
				BufferedReader reader = new BufferedReader(
						new InputStreamReader(mWorkerProc.getInputStream()));
				...
				try {
					...
					logger.log(Level.SEVERE,
							"Error reading from worker process stdout/stderr",
							e);
				}
			}
		}
	...
	/**
	 * @param workerProcDesc
	 *            the workerProcDesc to set
	 */
	public synchronized void setWorkerProcDesc(String workerProcDesc) {
		mWorkerProcDesc = workerProcDesc;
	}

	protected abstract Process startWorkerProcess() throws IOException;
	...
	public void execute() {
		...
		try {
			// 1. kick off
			mWorkerProc = startWorkerProcess();
			...
			if (exit_code == 0) {
				...
				result.setReason(getWorkerProcDesc() + " exited with "
						+ exit_code);
			}
			...
			// Cancelled by user
			logger.log(Level.SEVERE, "Interupted while executing "
					+ getWorkerProcDesc(), e);
			...
			logger.log(Level.SEVERE, "Error reading from "
					+ getWorkerProcDesc() + " stdout/stderr", e);
			result = WorkFlowResult.FAILED;
			result.setReason(getWorkerProcDesc() + " failed with "
					+ e.getMessage());
		} finally {
