	...
	private void init() {
		mLaunchManager = new FullLaunchManager(mConfig,
				new WorkFlowProgressListener() {
					...
					@Override
					public void onProgress(WorkFlowStage sender, WorkFlowStatus stage, int progress) {
						progressBarTotal.setSelection(stage.ordinal());
						progressBarStage.setSelection(progress);
					}

					@Override
					public void onCompleted(WorkFlowStage sender,
							WorkFlowStatus stage, WorkFlowResult result) {
						progressBarTotal.setSelection(stage.ordinal());
